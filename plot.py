...
formula_map = {
    r'dense_X_dense': formula_dense_dense,
    r'csr_X_dense':   formula_csr_dense,
    r'csc_X_dense':   formula_csc_dense,
    r'csr_X_csr':     formula_csr_csr,
}
...
plt.subplot(1,2,1)
plt.plot(densities, observed_flops, 'o', label='Observed', markersize=4)
plt.plot(densities, formula_flops, '-', label='Formula', linewidth=2)
plt.xlabel('Density')
plt.ylabel('Flops')
plt.title(f'{csv_file}: Flops')
plt.legend()
plt.grid(True)

plt.subplot(1,2,2)
plt.plot(densities, observed_memops, 'o', label='Observed', markersize=4)
plt.plot(densities, formula_memops, '-', label='Formula', linewidth=2)
plt.xlabel('Density')
plt.ylabel('Memops')
plt.title(f'{csv_file}: Memops')
plt.legend()
plt.grid(True)
